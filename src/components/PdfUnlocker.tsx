...
import {
  decryptPDF as decrypt,
  isEncrypted,
} from "@localonlytools/pdf-decrypt";
...
export const PdfUnlocker: React.FC = () => {
  ...
  const handleUnlock = async () => {
    ...
    try {
      ...
      if (!encrypted) {
        throw new Error(
          "This PDF is not encrypted or does not require a password.",
        );
      }
      ...
      const fixedBytes = new Uint8Array(savedBytes);
      ...
      if (
        ...
        setError(
          err.message ||
            "Failed to unlock PDF. The file might be corrupted or uses an unsupported encryption format.",
        );
      }
    } finally {
